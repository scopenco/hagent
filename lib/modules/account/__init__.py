...
def delete(args, config):
    '''Delete account.'''
    return Account(config, args.get('account')).delete(
        restart=args.get('restart'))


def lock(args, config):
    '''Lock account.'''
    return Account(config, args.get('account')).lock(
        'off', restart=args.get('restart'))


def unlock(args, config):
    '''Unlock account.'''
    return Account(config, args.get('account')).lock(
        'on', restart=args.get('restart'))


def preset(args, config):
    '''Change preset for account.'''
    return Account(config, args.get('account')).preset(
        args.get('preset'), restart=args.get('restart'))
